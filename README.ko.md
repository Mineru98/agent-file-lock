...
한 번 충분히 논의했고 승인을 받았고 그 뒤로는 의도적으로 건드리지 않기로 했습니다.
저희 팀에도 그런 문서가 몇 개 있었습니다. 어떤 것이 거기에 해당하는지 모두가
알고 있었고, 몇 달 동안 아무도 손대지 않았습니다.
...
그러나 그 편집들이 모여서 규칙 하나를 조용히 다시 써 버렸습니다. 사람이 의도를 가지고
적어 놓은 규칙이었습니다. 그리고 그 변경 사항은 아무도 자세히 들여다보기 전까지 한동안
브랜치에 그대로 남아 있었습니다.

불편한 대목은 따로 있었습니다. 잘못된 일은 아무것도 벌어지지 않았습니다. 에이전트는 어떤
규칙도 어기지 않았습니다. 에이전트가 볼 수 있는 규칙 자체가 없었기 때문입니다. 기여 안내서에
적어 둔 문구는 부탁입니다. `CLAUDE.md`에 써 놓은 한 줄도 부탁입니다. `chmod a-w`는
그보다 못한 수준인데, 다음 도구 호출이 동일한 사용자 권한으로 실행되므로 되돌렸다고
알리지도 않고 그대로 해제할 수 있기 때문입니다. 저희에게는 권고밖에 없었습니다. 그리고
도움이 되려고 애쓰는 무언가는 권고부터 가장 먼저 최적화해서 없앱니다.

그래서 보장은 에이전트가 손댈 수 없는 곳에서 나와야 했고, 동시에 설명을 동반해야
했습니다. 이 도구의 발상은 딱 여기까지입니다. 파일을 여러분의 권한으로 실행되는 그 어떤
것도 닿지 못하는 자리에 놓습니다. 그럼에도 무언가가 그 파일에 손을 뻗으면, 커널이 어쩔 수
없이 내놓은 숫자 대신에 사람이 썼을 법한 문장으로 대답합니다.

## 무엇을 하는 도구인가

`afl`은 코딩 에이전트(또는 여러분의 사용자 권한으로 실행되는 누구든)가 절대 수정해서는
안 되는 파일을 고정합니다. 커널의 **immutable 플래그**를 사용하는데, Linux에서는
`chattr +i`이고 macOS에서는 `chflags schg`입니다. 따라서 `chmod`와 달리 루트 권한
없이는 잠금을 해제할 수 없습니다. `chmod`는 같은 사용자가 간단히 되돌립니다. 잠긴
파일은 삭제하거나 이름을 바꾸는 것도 불가능하므로, 편집기와 에이전트가 즐겨 쓰는
"임시 파일에 쓴 다음 이름을 바꾸는" 방식도 통하지 않습니다.
...
자세한 내용은 [부모 디렉터리 가드](#부모-디렉터리-가드)에 있습니다.

그리고 스스로 이유를 설명합니다. 커널은 `EPERM` 하나로만 대답합니다.
"Operation not permitted"를 읽은 에이전트는 쓰기가 실패했다는 것까지만 압니다. 사람이
그렇게 되어서는 안 된다고 결정해 두었다는 것은 모릅니다. 앞에서 이야기한 우회 방법이
바로 이 지점에서 고안됩니다. `afl hook`은 도구 호출보다 먼저 실행되어 그 호출을 말로
거절합니다. 이 이야기는 [에이전트에게 이유를 알려 주기](#에이전트에게-이유를-알려-주기)에서
다룹니다.
...
설치합니다. 그 디렉터리에 실제로 권한이 필요할 때에만 `sudo`를 사용합니다. 환경 변수는
세 개를 읽습니다. `AFL_VERSION`은 특정 태그를 지정하고, `AFL_BIN_DIR`은 바이너리를
다른 곳에 두며, `AFL_NO_SUDO=1`은 권한을 올리는 대신 실패하게 만듭니다.
...
그다음 현재 어떤 빌드를 쓰고 있는지 확인하십시오. 새 릴리스가 이전 것보다 못하다고 판단되면
...
두 겹의 방어선이 있으며, 둘은 서로 반대 방향으로 작동합니다. 쓰기는 커널 플래그가 막고
그 이유는 훅이 설명합니다. 둘 중 하나만 있으면 빈틈이 생깁니다. 설명이 없는 플래그는
우회를 부추기고, 플래그가 없는 설명은 그저 권유에 그칩니다.
...
파일을 먼저 작성하고 그다음에 잠그십시오. 커널 플래그는 여러분을 포함한 모든
...
그다음 훅을 설치합니다. **이 저장소에서 에이전트가 작업한다면 이 단계를 건너뛰지
마시기 바랍니다.** 직접 실행하기 전에는 아무것도 등록되지 않으며, 커널의 응답만 가지고
판단하는 에이전트는 `Operation not permitted`를 고장 난 도구로 읽습니다. 그리고 바로
그때부터 우회로를 찾기 시작합니다. 에이전트마다 자기 설정 파일을 읽으므로, 실제로
쓰시는 쪽을 설치하면 됩니다.
...
내용에 병합할 뿐입니다. 실제로 적용되었는지 확인해 보십시오.
...
권한이 필요 없습니다. `.git`, `node_modules` 같은 디렉터리는 건너뛰는데 저장소의 크기만
키울 뿐 잠금이 걸릴 일은 없기 때문입니다(`-a`를 주면 포함하고 `--depth <n>`으로 탐색
깊이를 제한할 수 있습니다).
...
- 요청한 항목을 전부 건너뛰게 된 경우(예를 들어 보호 대상 파일이 심볼릭 링크로 바뀌어 있는 경우), `lock`은 exit 1로 끝나고 `check`는 겉으로만 성공한 결과 대신 이탈을 보고합니다.
...
- 감수해야 하는 비용도 분명히 있습니다. 가드가 걸려 있는 동안에는 그 디렉터리들 안의 *어떤* 항목도 삭제하거나 이름을 바꿀 수 없습니다. `rm -rf project`가 실패하고 `git checkout`도 실패합니다. `git checkout`은 최상위 파일을 임시 파일 생성 후 이름 변경 방식으로 교체하기 때문입니다. `sudo afl run -f afl.yaml -- git pull`이 이 문제를 처리해 주며(명령이 실행되는 동안 가드를 잠시 풀어 줍니다), `--guard-root`로 영향 범위를 좁힐 수 있습니다.
- Linux에는 사용자가 해제할 수 있는 append 플래그가 없으므로, `--level user`로 잠갔을 때의 부모 가드는 macOS와 BSD에서만 동작합니다. Linux에서는 그 사실을 보고하고 건너뜁니다.

## 에이전트에게 이유를 알려 주기

편집 도구에서 `EPERM`을 받은 에이전트는 쓰기가 실패했다는 것까지만 전달받습니다. 사람이
그렇게 되어서는 안 된다고 결정해 두었다는 말은 어디에도 없으며 이 차이가
...
훅에는 권한이 필요하지 않으며 이것은 두 번째 방어선입니다.
쓰기는 어느 쪽이든 커널이 거절하고 훅은 거기에 이유를 덧붙입니다.
...
읽는다면 `--format exit-code`를 쓰십시오. 하네스가 JSON을 넘겨줄 수 없을
...
함께 거절되므로, 그런 기능이 필요하시면 JSON을 쓰시면 됩니다. 예시는
[`afl.yaml.example`](afl.yaml.example)에 있습니다.
...
필요합니다(루트에는 있으나 Docker의 기본 capability 집합에는 **없습니다**.
